...
└── skills/
    ...
    │   ├── resources/             ← 按需加载的详细流程
    │   │   ├── doc-guide.md              ← 架构总览 + 职责 + 适配（入口索引）
    │   │   ├── tech-stack-matrix.md      ← 清点命令 / 锚点矩阵 / 模块识别策略
    │   │   ├── fact-classification.md    ← 填充原则 + T1/T2/T3 事实分类
    │   │   ├── front-matter-spec.md      ← YAML front matter + 结构化 TODO 枚举
...
blyy-init-docs/SKILL.md（~470 行，约 6.7K tokens，每次必加载）
    ...
    │     └─ Read resources/fact-classification.md（~80 行）
    │
    ├─ Phase 1 骨架生成 / 占位符填写（YAML front matter / TODO 枚举）
    │     └─ Read resources/front-matter-spec.md（~160 行）
    │

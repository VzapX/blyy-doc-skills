...
└── modules/       ← 模块单文件模板（`_module.md.template`，5 章节：概述/职责与边界/依赖关系/核心业务流程/代码锚点）
```
...
| `<!-- TODO[priority,type,owner]: 简述 -->` | 结构化 TODO 标记 |
...
audience: ai-only                 # 取值: ai-only | human-only | ai-and-human
read_priority: 2                  # 取值: 1-4，对应 Layer 1-3 渐进式披露（4 = 按需查阅）
max_lines: 400                    # 软上限（模块型推荐 ≤400），超出建议拆分
...
`skills/blyy-init-docs/resources/tech-stack-matrix.md` 中的「锚点文件矩阵」定义了各技术栈的关键文件识别规则（v1.0.0 起从 `doc-guide.md` 拆分出来）。

### 添加新技术栈

1. 在 `tech-stack-matrix.md` 三、「技术栈识别」表中添加新行
2. 在四、「锚点文件矩阵」中添加对应的后端/前端锚点
3. 在五、「模块识别策略」中补充该技术栈的模块识别特征
4. 在一、「确定性清点命令矩阵」中添加 `fd`/`rg` 命令（供 Phase 2 预扫描和 doc-sync 防线 3 使用）
...
| 文件 | 何时被读取 |
|------|-----------|
| `doc-guide.md` | Phase 1/Phase 3（架构总览、项目类型适配，入口索引） |
| `tech-stack-matrix.md` | Phase 1 项目类型识别 / Phase 2 扫描前（清点命令、锚点、模块识别） |
| `fact-classification.md` | Phase 2 子代理分发前（填充原则、T1/T2/T3） |
| `front-matter-spec.md` | Phase 1 骨架生成 / Phase 2 占位符填写（YAML front matter、TODO 枚举） |
...
| 场景 | 修改文件 |
|------|---------|
| 新增项目类型枚举（如 `mobile-app`） | `skills/blyy-init-docs/resources/doc-guide.md` 五、项目类型适配章节 + `front-matter-spec.md` 项目类型枚举 + 各 template 的 `applicable_project_types` 字段 |
...
| 调整字段说明提取优先级或补充新技术栈注释来源 | `skills/blyy-init-docs/resources/tech-stack-matrix.md` 二、字段说明提取矩阵 + `fact-classification.md` 一.2 规则 |
| 自定义模块单文件结构（5 章节排列） | `skills/blyy-init-docs/templates/modules/_module.md.template` |

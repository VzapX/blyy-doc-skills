...
├── INDEX.md.template              ← 任务路由表 + 新鲜度概览
├── modules.md.template            ← 模块注册表（按分级分组）
├── glossary.md.template           ← 业务术语 ↔ 代码符号映射
├── flows.md.template              ← 跨模块业务流程
├── decisions.md.template          ← 设计决策 + 不变式
├── code-queries.md.template       ← fd/rg 查询配方库
└── MANIFEST.yaml.template         ← 状态契约
```

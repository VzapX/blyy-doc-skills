...
└── docs/
    ...
    ├── api-reference.md     ← 全局公共信息（Base URL/认证/错误码）+ 模块索引（按需）
    ├── database/            ← 公共/跨模块表
    ├── modules/<m>/
    │   ├── README.md        ← 含入向/出向依赖
    │   ├── flow.md          ← 步骤含代码位置列
    │   ├── data-model.md    ← 字段说明自动从代码注释提取
    │   ├── api-reference.md ← 本模块接口详情（有 API 的模块）
    │   ├── code-map.md
    │   └── database/
...
| 新增功能 | `features.md` |
| API 端点变化（模块内） | `modules/<m>/api-reference.md` + `api-reference.md`（索引数字） |

...
`glossary.md` 的「字段业务语义」表只记录**字段名推断不出业务含义**的字段（如 Status 枚举值、Type 编码、复杂业务约束）。常见字段（id、created_at 等）不要列入。按以下优先级提取业务含义：
...
3. 迁移文件中的列注释
4. 字段名语义推断（T2 级别）
5. 无法确定时标注 `<!-- TODO[p2,business-context,user]: 请补充字段业务含义 -->`

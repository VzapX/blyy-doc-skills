...
2. **字段说明必填**：`data-model.md` 中所有表结构详情表的「说明」列**禁止留空**。提取优先级与各技术栈注释来源矩阵详见 `tech-stack-matrix.md` 二、字段说明提取矩阵。无法确定时标注 `<!-- TODO[p2,business-context,user]: 请补充字段说明 -->`（结构化 TODO 枚举定义见 `front-matter-spec.md`）。

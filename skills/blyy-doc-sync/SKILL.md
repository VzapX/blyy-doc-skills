...
> - 两者共享 `doc-guide.md` 中的确定性清点命令矩阵和技术栈锚点矩阵
...
对变更涉及的文档类别执行**针对性确定性扫描**（命令参考 `blyy-init-docs` 的 `resources/doc-guide.md` 确定性清点命令矩阵）：
...
2. **快速重评分**：对该模块执行复杂度评分（规则见 `blyy-init-docs/resources/doc-guide.md` 模块复杂度评分规则）
3. **比对**：
   - 评分结果与当前级别一致 → 无操作
...
2. 用 `blyy-init-docs/resources/doc-guide.md` 的确定性清点命令矩阵执行全量重扫
...
确定性扫描命令参考：
```
1. blyy-init-docs/resources/doc-guide.md  ← 确定性清点命令矩阵 + 技术栈锚点矩阵
```
...
- 确定性扫描命令需根据项目技术栈调整（参考 `doc-guide.md` 清点命令矩阵）
